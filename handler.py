...
def do_data_processing(fetch_result, cfg, url):
    ...
    xls_filepath = os.path.join(cfg["temp_path"], cfg['xls_filename'])
...
def scrape_repo_sofr(event, context):
    ...
    if is_result_valid:        
        df = do_data_processing(fetch_result, cfg, url)
        do_generate_output(df, cfg, url)
    else:
